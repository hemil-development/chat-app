...
const HEMIL_COMPANY_USER_ID = '6f1f5d44-820e-4dda-960e-b0926f7f76e7';
    ...
    async function loadData() {
      try {
        ...
        const { data: cUsers } = await supabase
          .from('company_users')
          .select('*, users(*)')
          .neq('id', HEMIL_COMPANY_USER_ID);

        // 3. Fetch rooms
        const { data: rooms } = await supabase
          .from('chat_rooms')
          .select('*')
          .contains('participants', [HEMIL_COMPANY_USER_ID]);
        ...
        const formatLastMsg = (msg) => {
          if (!msg) return '';
          if (msg.created_by === HEMIL_COMPANY_USER_ID) {
        ...
        const directContacts = (cUsers || []).map(cu => {
          const matchingRoom = (rooms || []).find(r => 
            r.type === 'single' && 
            r.participants.includes(HEMIL_COMPANY_USER_ID) && 
          ...
          return {
            ...
            lastMessage: lastMsg ? (lastMsg.created_by === HEMIL_COMPANY_USER_ID ? `You: ${lastMsg.message}` : lastMsg.message) : 'Click to start chatting',
            timestamp: lastMsg ? formatMessageTime(lastMsg.created_at) : '',
            unread: 0,
            pinned: false,
            isChannel: false,
        ...
        try {
          const { data: dbNotifs } = await supabase
            ...
            `)
            .eq('recipient_id', HEMIL_COMPANY_USER_ID)
  ...
  const sendTypingStatus = (isTyping) => {
    if (activeChannelRef.current) {
      activeChannelRef.current.send({
        type: 'broadcast',
        event: 'typing',
        payload: { userId: HEMIL_COMPANY_USER_ID, isTyping },
      });
    }
  };
